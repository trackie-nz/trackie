...
#      the normal build): the /get-started route, the HMAC identity module, and
#      the deny-by-default /admin guard.
...
#
# This overlay is CORE ONLY: privacy sign-in (HMAC identity + reject unverified
# email), the deny-by-default /admin gateway, and the /get-started deep link.
# Akahu is deferred and lives dormant in ../akahu-overlay/ - see its ENABLE.md.
