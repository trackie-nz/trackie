...
| `branding.patch` | `desktop-client/index.html` + `public/site.webmanifest` | Rebrands the browser tab title and PWA `name`/`short_name` from "Actual" to "Trackie". Also adds an inline in-app-webview guard to the `<head>`: when cross-origin isolation / `SharedArrayBuffer` is missing (Facebook/Instagram/etc. in-app browsers) it overlays a self-contained "open in your browser" screen at max `z-index` over the app root, instead of leaving the user on Actual's scary SharedArrayBuffer "Fatal Error". The overlay is built with DOM APIs only (no HTML string), deliberately: Vite (build) and Cloudflare (edge) inject `<script>`/`<link>` tags keyed off the head/body close tags, so an HTML string here would get tags spliced into it and have its `<script>` prematurely closed. |
