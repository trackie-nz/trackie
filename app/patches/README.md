...
| `branding.patch` | `desktop-client/index.html` + `public/site.webmanifest` | Rebrands the browser tab title and PWA `name`/`short_name` from "Actual" to "Trackie". Also adds an inline in-app-webview guard to the `<head>`: it runs before the (deferred) app bundle and, when cross-origin isolation / `SharedArrayBuffer` is missing (Facebook/Instagram/etc. in-app browsers), stops the load and shows a self-contained "open in your browser" screen instead of Actual's scary SharedArrayBuffer "Fatal Error". |
