...
| --- | --- |-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `akahu-app.patch` | `sync-server/.../app-akahu/app-akahu.ts` | Makes upstream's released Akahu router per-user with a surgical hook, *not* a fork: swaps the three `secretsService.get` token reads for `getUserTokens(res.locals.user_id)`, mounts the `/tokens` set/clear routes (both from the `trackie-akahu.ts` drop-in, no upstream transaction-processing code is copied), and widens upstream's own on-demand refresh cap from 1h to 20h (one constant). |
| `akahu-auto-sync-financesapp.patch` | `desktop-client/.../components/FinancesApp.tsx` | Calls the `useAkahuAutoSync` drop-in hook (two lines: import + call) so linked bank accounts auto-sync on budget open (logic lives in the `hooks/useAkahuAutoSync.ts` drop-in). Also rebrands the update-notification title/message from "Actual" to "Trackie" - the service-worker "A new version … is available!" popup shown on each deploy (folded in here to keep one patch per file). Display-only. |
...
| `akahu-loot-core.patch` | `loot-core/.../accounts/app.ts` | The only patch on this file (one-patch-per-file). Adds the per-user `akahu-set-tokens` / `akahu-clear-tokens` handlers (POST/DELETE to `AKAHU_SERVER + '/tokens'`) plus their type entries and the `del` import. Upstream already registers its own `akahu-accounts` method from v26.7.0, so we no longer re-add it. |
| `akahu-providers.patch` | `desktop-client/.../banksync/useBuiltInBankSyncProviders.ts` | Makes the Akahu card configurable by ordinary users (`canConfigure: true`, since each user manages their own personal token - other providers stay admin-gated), and repoints its reset to a single `akahu-clear-tokens` call instead of two admin-only `secret-set` clears. |
