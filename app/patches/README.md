...
| --- | --- |-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `akahu-6041-appmethod.patch` | `loot-core/.../accounts/app.ts` | Re-adds the one hunk that rejects when `apply-overlay.sh` backports upstream Akahu PR #6041 (step 0): the `app.method('akahu-accounts', akahuAccounts)` registration. The surrounding simplefin/gocardless lines drifted after v26.6.0 (`accounts-bank-sync` lost its `mutator()` wrapper), so this is authored against the *post-#6041* tree. Retire it together with the step-0 backport when a released tag ships #6041. |
